...
image:cloud-fabricpool_image10.png["유효하고 유효하지 않은 FabricPool 애그리게이트-버킷 연결 구성을 보여주는 다이어그램"]
...
+
image:cloud-fabricpool_image11.png["클라우드의 S3 확장 가능 스토리지 결과가 강조 표시된 AWS 콘솔 검색 스크린샷"]

. S3 홈 페이지에서 Create Bucket(버킷 생성) 을 선택합니다.
. DNS 호환 이름을 입력하고 버킷을 생성할 지역을 선택합니다.
+
image:cloud-fabricpool_image12.png["버킷 이름, 리전 및 생성 버튼이 있는 AWS 버킷 생성 대화 상자의 스크린샷"]
