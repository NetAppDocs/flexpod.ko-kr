...
+
image:express-c-series-c190-deploy_image17.png["상단 메뉴에 KVM 시작 링크가 있는 Cisco CIMC 서버 BIOS 페이지의 스크린샷"]
...
=== ESXi 6.7U2 Cisco 사용자 지정 이미지 다운로드
...
+
image:express-c-series-c190-deploy_image18.png["15.00 GiB 용량의 NETAPP LUN C-Mode 스토리지 장치를 선택하는 ESXi 설치 프로그램의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image19.png["관리 네트워킹을 위해 vmnic2와 vmnic4가 선택된 ESXi 네트워크 어댑터 화면의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image20.png["vSwitch0 및 iScsiBootvSwitch를 보여주는 가상 스위치 탭이 있는 ESXi 네트워킹 페이지의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image21.png["vmnic3이 활성화되고 vmnic5가 사용되지 않는 iSCSIBootPG-A 팀 구성 및 페일오버 설정의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image22.png["iSCSI 소프트웨어 어댑터 vmhba64가 선택된 ESXi 스토리지 어댑터 탭의 스크린샷"]

. 동적 대상에서 동적 대상 추가를 클릭합니다.
+
image:express-c-series-c190-deploy_image23.png["4개의 IP 주소와 포트 3260이 구성된 iSCSI 동적 타겟을 보여주는 스크린샷"]
...
.. 구성 저장 을 클릭합니다.
+
image:express-c-series-c190-deploy_image24.png["포트 3260을 사용하는 4개의 iSCSI 동적 타겟 주소와 구성 저장 버튼의 스크린샷"]
...
. vSwitch0을 선택합니다.
+
image:express-c-series-c190-deploy_image25.png["MTU 9000 및 활성 vmnic2, vmnic4 어댑터를 보여 주는 ESXi vSwitch0 페이지의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image26.png["포트 그룹 탭이 선택되어 있고 포트 그룹 추가 옵션이 있는 ESXi 네트워킹 페이지의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image27.png["vmk0 및 vmk1 인터페이스 목록이 표시된 ESXi 네트워킹 VMkernel NIC 탭의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image28.png["고정 IPv4 설정 및 vMotion이 선택된 상태에서 VMkernel NIC 추가 대화 상자의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image29.png["새 데이터스토어 버튼이 표시된 VMware ESXi 스토리지 페이지의 스크린샷"]

. Mount NFS Datastore를 선택합니다.
+
image:express-c-series-c190-deploy_image30.png["생성 유형으로 NFS 데이터 저장소 마운트가 선택된 새 데이터 저장소 마법사의 스크린샷"]
...
. 저장 을 클릭합니다.
+
image:express-c-series-c190-deploy_image31.png["NTP 서비스가 실행 중이고 3개의 NTP 서버가 있는 ESXi 시간 및 날짜 설정을 보여주는 스크린샷"]
...
+
image:express-c-series-c190-deploy_image32.png["VMware ESXi 시스템 스왑 관리 페이지의 스크린샷으로, 스왑 설정이 표시되어 있습니다."]

. 설정 편집 을 클릭합니다. Datastore 옵션에서 infra_swap을 선택합니다.
+
image:express-c-series-c190-deploy_image33.png["infra_swap이 데이터스토어로 선택되고 저장 및 취소 버튼이 있는 스왑 구성 편집 대화 상자의 스크린샷"]

. 저장 을 클릭합니다.


link:express-c-series-c190-deploy_vmware_vcenter_server_6.7u2_installation_procedure.html["다음: VMware vCenter Server 6.7U2 설치 절차"]
