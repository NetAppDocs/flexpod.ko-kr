...
[role="lead"]
FabricPool은 ONTAP 9.2 이상이 필요합니다. 추가 FabricPool 요구 사항은 사용 중인 ONTAP 버전과 연결된 클라우드 계층에 따라 다릅니다. ONTAP 9.8 이전 릴리즈에서는 FabricPool이 SSD 로컬 계층에서만 지원됩니다. HDD 애그리게이트에 대한 지원은 ONTAP 9.8에서 추가되었습니다. 고정적이고 비교적 작은 용량을 가진 엔트리급 AFF 플랫폼(예: NetApp AFF C190)의 경우, FabricPool은 비활성 데이터를 클라우드 계층으로 이동하는 데 매우 효과적일 수 있습니다.
...
* NetApp AFF를 참조하십시오
* NetApp FAS FabricPool은 ONTAP 9.2를 실행할 수 있는 모든 플랫폼에서 지원되지만, 다음 플랫폼은 제외됩니다.
+
** FAS8020
...
NOTE: FabricPool를 사용하는 지원되는 플랫폼에 대한 최신 정보는 https://hwu.netapp.com/DetailViews/Home.aspx["NetApp Hardware Universe를 참조하십시오"^]를 참조하십시오.

* NetApp ONTAP Select
+

NOTE: NetApp에서는 모든 SSD FabricPool 로컬 계층을 사용할 것을 권장합니다.
...
** NetApp StorageGRID 10.3 이상 버전
+

NOTE: Glacier Flexible Retrieval 및 Glacier Deep Archive는 지원되지 않습니다.
...
image:cloud-fabricpool_image6.png["두 대의 NetApp AFF A300 컨트롤러가 클러스터 간 LIF를 AWS S3에 연결하는 방식을 보여주는 다이어그램"]
...
image:cloud-fabricpool_image7.png["PUT 및 GET 지연 시간과 처리량을 보여주는 ONTAP CLI 오브젝트 저장소 프로파일러 결과의 스크린샷"]
...
FabricPool 라이선스(기존 라이선스에 대한 추가 용량 포함)는 1TB 단위로 구매할 수 있습니다.
