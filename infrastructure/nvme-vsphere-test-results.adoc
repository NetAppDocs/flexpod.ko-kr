...
image:nvme-vsphere-image3.png["FlexPod AFF A800 8KB 랜덤 읽기 IOPS와 지연 시간 간의 관계를 나타낸 그래프로, 0.76ms에서 125만 IOPS에 도달했습니다."]
...
image:nvme-vsphere-image4.png["FlexPod AFF A800 8KB 랜덤 쓰기 지연 시간이 IOPS에 따라 증가하는 것을 보여주는 그래프"]

이 테스트에서는 약 300,000 IOPS를 달성했지만 1ms 미만의 서버 측 지연 시간만 유지된다는 사실을 발견했습니다.

8KB 블록 크기, 80% 랜덤 읽기 및 20% 쓰기의 경우 다음과 같은 결과를 확인했습니다.

image:nvme-vsphere-image5.png["FlexPod AFF A800 8KB 80:20 지연 시간이 1M IOPS에서 0.95ms까지 상승하는 것을 보여주는 차트"]

테스트에서는 시스템이 1백만 IOPS 이상을 달성하는 동시에 1ms 미만의 서버 측 지연 시간을 유지한다는 사실을 발견했습니다.

64KB 블록 크기 및 100% 순차적 읽기의 경우 다음 결과가 관찰되었습니다.

image:nvme-vsphere-image6.png["FlexPod AFF A800 IOPS가 증가함에 따라 FlexPod AFF A800의 64KB 순차 읽기 지연 시간이 0.2ms에서 0.78ms로 증가하는 것을 보여주는 차트"]

이번 테스트에서 시스템은 지연 시간이 1ms 미만인 동시에 약 250,000 IOPS를 실현했다는 사실을 확인했습니다.

64KB 블록 크기 및 100% 순차적 쓰기의 경우 다음 결과가 관찰되었습니다.

image:nvme-vsphere-image7.png["FlexPod AFF A800 64KB 순차 쓰기 지연 시간이 IOPS에 따라 증가하는 것을 보여주는 차트"]
