...
+
image:flexpod-genomics-image18.png["Cisco UCS Manager 인벤토리 GPU 탭에서 컴퓨트 모드의 P6 그래픽 카드를 보여주는 스크린샷"]

+
image:flexpod-genomics-image19.png["Compute 모드의 UCSB-GPU-P6-R이 포함된 GPU 인벤토리를 보여주는 Cisco UCS Manager 스크린샷"]
...
. 이 명령은 ESXi에  https://docs.nvidia.com/vgpu/latest/grid-software-quick-start-guide/installing-configuring-vgpu-manager-guest-driver.html#installing-the-nvidia-virtual-gpu-manager-on-vmware-vsphere["NVIDIA vGPU Manager"^]을 설치합니다.
+
....
...
+
image:flexpod-genomics-image20.png["Shared Direct로 구성된 NVIDIA Tesla P6 그래픽 장치를 보여주는 vSphere Client 스크린샷"]

+
image:flexpod-genomics-image21.png["공유 직접 연결(Shared Direct)이 선택되고 X.Org 서버 재시작(Restart X.Org server)이 체크된 상태에서 그래픽 장치 설정 편집(Edit Graphics Device Settings)의 스크린샷"]
...
+
image:flexpod-genomics-image22.png["EFI 펌웨어 및 보안 부팅이 비활성화된 것을 보여주는 vCenter VM 부팅 옵션의 스크린샷"]
...
+
image:flexpod-genomics-image23.png["Edit Configuration 링크가 있는 vCenter Edit Settings 고급 섹션을 보여주는 스크린샷"]

+
image:flexpod-genomics-image24.png["pciPassthru.64bitMMIOSizeGB 및 pciPassthru.use64bitMMIO 값이 나열된 구성 매개변수 대화 상자의 스크린샷"]
...
+
image:flexpod-genomics-image25.png["PCI 장치에 대해 NVIDIA GRID vGPU 프로필 grid_p6-16c가 선택된 것을 보여주는 vCenter 편집 설정의 스크린샷"]
...
+
image:flexpod-genomics-image26.png["가상 이더넷 클라이언트 하나가 나열된 NVIDIA License Server 라이선스 클라이언트 페이지의 스크린샷"]
