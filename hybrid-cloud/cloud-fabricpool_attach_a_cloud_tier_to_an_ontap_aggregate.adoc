...
. 애플리케이션 및 계층 을 클릭합니다.
+
image:cloud-fabricpool_image14.png["Applications  Tiers가 선택된 OnCommand System Manager 탐색 메뉴를 보여주는 스크린샷"]

. Storage Tiers를 클릭합니다.
. Aggregate를 클릭합니다.
. 작업 을 클릭하고 클라우드 계층 연결 을 선택합니다.
+
image:cloud-fabricpool_image15.png["클라우드 계층 연결이 선택된 추가 작업 메뉴를 보여주는 스크린샷"]
