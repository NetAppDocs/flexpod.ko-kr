...
image:sm-bcs-image39.png["멀티 사이트 패브릭을 통해 FlexPod 사이트 A 및 B에 걸쳐 있는 VMware HA 및 vSphere Metro Storage Cluster를 보여주는 다이어그램"]
...
image:sm-bcs-image40.png["사이트 A와 B에 있는 Cisco UCS B200 M5 서버와 AFF A250 스토리지가 Nexus 스위치를 통해 연결되어 있고, 사이트 C에는 ONTAP Mediator가 설치되어 있는 것을 보여주는 다이어그램입니다."]
...
image:sm-bcs-image41.png["vSwitch0, vDS0, iSCSI-A 및 iSCSI-B 스위치와 6개의 vNIC 업링크를 보여주는 다이어그램"]
...
image:sm-bcs-image42.png["사이트 A 및 사이트 B 호스트와 VM에 대한 VM/호스트 그룹을 보여 주는 vSphere의 스크린샷"]

또한 다음 두 그림에서는 사이트 A 및 사이트 B VM이 "그룹의 호스트에서 실행해야 함" 정책을 사용하여 각 사이트의 호스트에서 실행될 수 있도록 만들어진 VM/호스트 규칙을 보여 줍니다.

image:sm-bcs-image43.png["사이트 A VM이 사이트 A 호스트에서 실행되도록 구성하는 VM/호스트 규칙 편집 대화 상자의 스크린샷"]

image:sm-bcs-image44.png["그룹 정책에서 실행되어야 하는 호스트가 포함된 Site B VM 및 호스트에 대한 VM/호스트 규칙 편집 대화 상자의 스크린샷"]
...
image:sm-bcs-image45.png["4개의 하트비트 데이터스토어와 2개의 격리 주소가 구성된 vSphere HA 고급 옵션의 스크린샷"]

하트비트 데이터 저장소의 경우 다음 그림과 같이 클러스터에서 4개의 공유 데이터 저장소를 지정하고 자동으로 보완합니다.

image:sm-bcs-image46.png["네 개의 지정된 데이터스토어가 선택된 vSphere HA Heartbeat 데이터스토어 설정 스크린샷"]

VMware HA 클러스터 및 VMware vSphere Metro 스토리지 클러스터에 대한 추가 모범 사례 및 구성은  https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.avail.doc/GUID-5432CA24-14F1-44E3-87FB-61D937831CF6.html["vSphere HA 클러스터 생성 및 사용"^],  https://core.vmware.com/resource/vmware-vsphere-metro-storage-cluster-vmsc["VMware vMSC(vSphere Metro Storage Cluster)"^] 및  https://docs.netapp.com/us-en/netapp-solutions-virtualization/vmware/vmw-vmsc-with-smas.html["NetApp ONTAP with SnapMirror active sync(이전 SM-BC) 및 VMware vSphere Metro Storage Cluster(vMSC)"^]을 참조하십시오.
