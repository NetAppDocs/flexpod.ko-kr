...
+
.. 리포지토리 이름 * 에서 섹션에 자세히 설명된 리포지토리 이름을 입력합니다 link:p-ncvo-automated-deployment-of-hybrid-cloud-netapp-storage.html#google-cloud#set-up-environment-prerequisites[""환경 필수 구성 요소 설정""].
.. 섹션에 자세히 설명된 대로 OAuth 토큰 ID를 제공합니다 link:p-ncvo-automated-deployment-of-hybrid-cloud-netapp-storage.html#google-cloud#set-up-environment-prerequisites[""환경 필수 구성 요소 설정""].
.. 자동 트리거 실행 * 옵션을 선택합니다.
+
...
NOTE: NetApp Cloud Manager API 작업을 위한 업데이트 토큰을 얻는 방법에 대한 자세한 내용은 섹션을 참조하십시오 link:p-ncvo-automated-deployment-of-hybrid-cloud-netapp-storage.html#google-cloud#set-up-environment-prerequisites["“환경 필수 구성 요소 설정.”"]
