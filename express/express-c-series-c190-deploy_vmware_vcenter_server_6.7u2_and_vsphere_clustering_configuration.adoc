...
. 다음으로 이동하세요. `+https://<<FQDN or IP of vCenter>>/vsphere-client/+`
...
. 확인 을 클릭합니다.
+
image:express-c-series-c190-deploy_image45.png["DRS 및 vSphere HA가 활성화되고 vSAN이 비활성화된 새 클러스터 대화 상자의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image46.png["호스트 FQDN 입력 필드와 다음 버튼이 있는 vSphere Client 호스트 추가 마법사의 스크린샷"]
...
.  `https://<vCenter IP>:5480`에 로그인하고 사용자 이름에 root를 입력한 다음 root 암호를 입력하십시오.
. 서비스를 클릭하고 VMware vSphere ESXi Dump Collector를 선택합니다.
. VMware vSphere ESXi Dump Collector 서비스를 시작합니다.
+
image:express-c-series-c190-deploy_image47.png["VMware vSphere ESXi 덤프 수집기가 선택되고 시작 옵션이 활성화된 어플라이언스 관리 서비스의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image48.png["ESXi 코어덤프 구성 명령 및 netdump 서버 검증 결과를 보여주는 터미널 화면"]
