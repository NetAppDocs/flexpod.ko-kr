...
summary: 이 섹션에서는 Cromwell 엔진을 사용하여 GATK를 실행한 후의 출력을 보여 줍니다. 
---
= Cromwell 엔진을 사용하여 GATK 실행을 위한 출력
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
link:flexpod-genomics-appendix-b.html["이전: './GATK' 스크립트를 사용한 GATK 실행 출력"]
...
....
link:flexpod-genomics-appendix-d-gpu-setup.html["다음: GPU 설정."]
