...
+
image:express-c-series-aff220-deploy_image21.png["상단 메뉴에 KVM 시작 링크가 있는 Cisco CIMC 서버 BIOS 페이지의 스크린샷"]
...
=== ESXi 6.7 Cisco 사용자 지정 이미지 다운로드

. 사용자 지정 ISO의 https://support.broadcom.com/group/ecx/downloads["Broadcom 지원 포털 다운로드 사이트"^]로 이동합니다.
...
+
image:express-c-series-aff220-deploy_image22.png["15.00 GiB 용량의 NETAPP LUN C-Mode 스토리지 장치를 선택하는 ESXi 설치 프로그램의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image23.png["vSwitch0에 대해 vmnic0 및 vmnic1이 선택된 ESXi 네트워크 어댑터 메뉴의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image24.png["vSwitch0 및 iScsiBootvSwitch를 보여주는 ESXi 네트워킹 가상 스위치 탭의 스크린샷"]
...
** 생성 을 클릭합니다.
+
image:express-c-series-aff220-deploy_image25.png["고정 IPv4 설정 및 MTU 9000으로 iScsiBootPG-B를 구성하는 VMkernel NIC 추가 양식의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image26.png["iSCSI 소프트웨어 어댑터 vmhba64가 선택된 VMware ESXi 스토리지 어댑터 페이지의 스크린샷"]

. 동적 대상에서 동적 대상 추가를 클릭합니다.
+
image:express-c-series-aff220-deploy_image27.png["동적 대상 아래에 동적 대상 추가가 표시된 iSCSI 구성 대화 상자의 스크린샷"]
...
** 구성 저장 을 클릭합니다.
+
image:express-c-series-aff220-deploy_image28.png["포트 3260을 각각 사용하는 4개의 iSCSI 동적 타겟 IP 주소를 보여주는 스크린샷"]
...
. vSwitch0을 선택합니다.
+
image:express-c-series-aff220-deploy_image29.png["MTU 1500 및 두 개의 활성 물리적 어댑터를 보여주는 VMware ESXi vSwitch0 페이지의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image30.png["포트 그룹 탭이 선택되어 있고 포트 그룹 추가 옵션이 있는 ESXi 네트워킹 페이지의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image31.png["vmk0 및 vmk1 어댑터 목록이 표시된 ESXi 네트워킹 VMkernel NIC 탭의 스크린샷"]
...
** 생성 을 클릭합니다.
+
image:express-c-series-aff220-deploy_image32.png["NFS 네트워크 포트 그룹을 고정 IPv4 설정으로 구성하는 VMkernel NIC 추가 양식의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image33.png["고정 IPv4 설정 및 vMotion이 선택된 상태에서 VMkernel NIC 추가 대화 상자의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image34.png["새 데이터스토어 버튼이 표시된 VMware ESXi 스토리지 페이지의 스크린샷"]

. Mount NFS Datastore를 선택합니다.
+
image:express-c-series-aff220-deploy_image35.png["생성 유형으로 NFS 데이터 저장소 마운트가 선택된 새 데이터 저장소 마법사의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image36.png["NTP 클라이언트 상태와 NTP 서버를 보여주는 ESXi 관리 시스템 시간 및 날짜 페이지의 스크린샷"]
...
. 저장 을 클릭합니다.
+
image:express-c-series-aff220-deploy_image37.png["NTP가 활성화되고 NTP 서버가 입력된 시간 구성 편집 대화 상자의 스크린샷"]
...
+
image:express-c-series-aff220-deploy_image38.png["VMware ESXi 시스템 스왑 관리 페이지의 스크린샷으로, 스왑 설정이 표시되어 있습니다."]

. 설정 편집 을 클릭합니다. Datastore 옵션에서 infra_swap을 선택합니다.
+
image:express-c-series-aff220-deploy_image39.png["infra_swap이 데이터스토어로 선택되고 저장 및 취소 버튼이 있는 스왑 구성 편집 대화 상자의 스크린샷"]
...
. ESXi 호스트를 재부팅합니다.
+
image:express-c-series-aff220-deploy_image40.png["NetApp NFS Plug-in VIB 설치 성공 및 재부팅 필요 메시지를 표시하는 터미널"]
