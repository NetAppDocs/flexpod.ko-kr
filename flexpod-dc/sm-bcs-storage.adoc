...
image:sm-bcs-image22.png["두 사이트에서 Nexus A 및 Nexus B 스위치에 연결된 AFF A250을 보여주는 다이어그램"]
...
image:sm-bcs-image23.png["Nexus에 연결된 AFF A250 컨트롤러 포트, 인터페이스 그룹, VLAN 포트 및 LIF를 보여주는 다이어그램"]
...
image:sm-bcs-image24.png["B200 M5 서버에서 AFF A250 컨트롤러로의 이중화된 iSCSI SAN 부팅 연결을 보여주는 다이어그램"]
...
image:sm-bcs-image25.png["두 개의 IP 주소 필드가 있는 ONTAP System Manager의 클러스터 간 인터페이스 추가 대화 상자 스크린샷"]

두 클러스터를 함께 클러스터하려면 다음 단계를 완료하십시오.

. 첫 번째 클러스터에서 클러스터 피어링 암호를 생성합니다.
+
image:sm-bcs-image26.png["클러스터 피어 권한, 유효성 및 복사된 암호가 포함된 암호 생성 대화 상자의 스크린샷"]

. 두 번째 클러스터에서 피어 클러스터 옵션을 호출하고 암호 구문 및 인터클러스터 LIF 정보를 제공합니다.
+
image:sm-bcs-image27.png["암호 및 인터클러스터 LIF 주소가 표시된 ONTAP System Manager 피어 클러스터 대화 상자를 보여주는 스크린샷"]

. System Manager 보호 > 개요 창에 클러스터 피어 정보가 표시됩니다.
+
image:sm-bcs-image28.png["클러스터 피어 및 중재자 상태를 보여주는 ONTAP System Manager 보호 개요 스크린샷"]
...
image:sm-bcs-image29.png["ONTAP System Manager의 미디에이터 구성 대화 상자의 스크린샷으로, 미디에이터 연결 필드가 표시됩니다."]

필요한 정보를 제공하면 구성된 ONTAP 중재자가 시스템 관리자 보호 > 개요 창에 나타납니다.

image:sm-bcs-image30.png["구성된 Mediator 상태를 보여주는 ONTAP System Manager 보호 개요 창의 스크린샷"]
...
image:sm-bcs-image31.png["소스 볼륨 esxi_a와 타겟 클러스터 aff-a250-b를 사용한 일관성 그룹 보호 대화 상자의 스크린샷"]
...
image:sm-bcs-image32.png["사이트 A에 있는 두 개의 정상적이고 동기화된 일관성 그룹을 보여주는 ONTAP System Manager 관계의 스크린샷"]

이 스크린샷은 사이트 B의 일관성 그룹 관계를 보여 줍니다

image:sm-bcs-image33.png["사이트 B의 ONTAP System Manager 관계를 보여주는 스크린샷으로, 두 개의 정상적이고 동기화된 일관성 그룹이 표시됩니다."]

이 스크린샷은 CG_infra_datastore_b 그룹에 대한 정합성 보장 그룹 관계 세부 정보를 보여 줍니다.

image:sm-bcs-image34.png["cg_infra_datastore_b 일관성 그룹 관계 세부 정보를 보여주는 ONTAP System Manager의 스크린샷"]
...
image:sm-bcs-image35.png["Infra-SVM-a의 ONTAP 볼륨 정보 스크린샷(RW, DP, LS 볼륨 포함)"]

이 스크린샷은 사이트 B의 볼륨 정보를 보여 줍니다

image:sm-bcs-image36.png["타겟 DP 볼륨을 포함하여 9개의 볼륨을 나열하는 Infra-SVM-b에 대한 ONTAP volume show 출력의 스크린샷"]
...
image:sm-bcs-image37.png["사이트 A의 iSCSI igroup 및 LUN 매핑을 ONTAP CLI 출력에서 보여주는 스크린샷"]

이 스크린샷은 사이트 B 클러스터에 대한 호스트 igroup 및 LUN 매핑을 보여 줍니다.

image:sm-bcs-image38.png["iSCSI VMware 호스트에 대한 사이트 B igroup 및 LUN 매핑 명령 출력의 스크린샷"]
