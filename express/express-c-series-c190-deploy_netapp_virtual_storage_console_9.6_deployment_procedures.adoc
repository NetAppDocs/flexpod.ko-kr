...
+
image:express-c-series-c190-deploy_image49.png["열기 대화 상자에서 VSC OVF 파일이 선택된 vSphere Client OVF 템플릿 배포의 스크린샷"]

. VM 이름을 입력하고 배포할 데이터 센터 또는 폴더를 선택합니다. 다음 을 클릭합니다.
+
image:express-c-series-c190-deploy_image50.png["VM 이름이 FlexPod-VSC이고 FlexPod-Datacenter가 선택된 OVF 템플릿 배포 마법사의 스크린샷"]

. FlexPod-Cluster ESXi 클러스터를 선택하고 Next를 클릭합니다.
. 세부 정보를 검토하고 Next를 클릭합니다.
+
image:express-c-series-c190-deploy_image51.png["NetApp VSC 가상 어플라이언스 OVF 템플릿을 배포하기 위한 세부 정보 검토 단계를 보여주는 스크린샷"]

. Accept(수락) 를 클릭하여 라이센스를 수락하고 Next(다음) 를 클릭합니다.
. Thin Provision 가상 디스크 형식과 NFS 데이터 저장소 중 하나를 선택합니다. 다음 을 클릭합니다.
+
image:express-c-series-c190-deploy_image52.png["Thin Provision 및 NFS 데이터스토어를 사용하는 OVF 템플릿 배포 스토리지 선택 페이지의 스크린샷"]

. 네트워크 선택에서 대상 네트워크를 선택하고 다음을 클릭합니다.
+
image:express-c-series-c190-deploy_image53.png["OVF 템플릿 배포 네트워크 선택 단계에서 nat을 MGMT-Network에 매핑하는 스크린샷"]

. 템플릿 사용자 지정에서 VSC 관리자 암호, vCenter 이름 또는 IP 주소, 기타 구성 정보를 입력하고 다음을 클릭합니다.
+
image:express-c-series-c190-deploy_image54.png["vCenter 등록 필드 및 네트워크 속성이 포함된 OVF 템플릿 배포 템플릿 사용자 정의 페이지의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image55.png["VSC, VASA Provider 및 SRA 설치를 계속하기 전에 VMware Tools를 설치하라는 지침을 보여주는 VM 콘솔"]
...
+
image:express-c-series-c190-deploy_image56.png["Virtual Storage Console이 강조 표시된 vSphere Client 메뉴의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image57.png["업로드를 위해 NetAppNasPlugin.vib 파일이 선택된 VSC NFS VAAI 툴의 스크린샷"]
...
+
image:express-c-series-c190-deploy_image58.png["VMware VAAI용 NFS 플러그인 설치가 선택된 NetApp VSC 메뉴를 보여주는 스크린샷"]
...
+
image:express-c-series-c190-deploy_image59.png["NetApp VSC Set Recommended Values가 선택된 vSphere Client 호스트 작업 메뉴의 스크린샷"]

. 선택한 vSphere 호스트에 적용할 설정을 선택합니다. 확인 을 클릭하여 설정을 적용합니다.
+
image:express-c-series-c190-deploy_image60.png["선택한 HBA/CNA 어댑터, MPIO 및 NFS 설정이 포함된 권장 값 설정 대화 상자"]

+
image:express-c-series-c190-deploy_image61.png["스토리지 시스템 검색이 성공적으로 완료되면 ESXi 호스트 설정이 수정되었다는 성공 대화 상자가 나타납니다."]
