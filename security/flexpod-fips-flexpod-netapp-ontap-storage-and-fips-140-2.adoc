...
image:flexpod-fips-image3.png["암호화된 쓰기 데이터와 복호화된 읽기 데이터가 네트워크, WAFL, RAID 및 암호화 모듈이 장착된 스토리지를 통해 흐르는 과정을 보여주는 다이어그램"]
...
image:flexpod-fips-image4.png["Network, WAFL, RAID 및 스토리지 암호화 모듈 계층을 통한 암호화 및 복호화를 보여주는 다이어그램"]
...
image:flexpod-fips-image5.png["키 관리자, cryptomod 및 cryptomod_fips 구성 요소를 포함하는 ONTAP 암호화 경계를 보여주는 다이어그램"]
