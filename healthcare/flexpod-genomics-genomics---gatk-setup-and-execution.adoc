...
image:flexpod-genomics-image8.png["샘플 수집부터 시퀀싱, 변이 분석, 전자건강기록(EHR) 통합 및 예측 모델에 이르는 유전체학 워크플로우를 보여주는 다이어그램"]
...
+
image:flexpod-genomics-image10.png["범위, 서브넷 마스크, 관리, VM, NFS 및 iSCSI 용도를 보여 주는 VLAN IP 예약 표"]

. ONTAP SVM에서 iSCSI 기반 부팅 LUN을 구성합니다.
+
image:flexpod-genomics-image9.png["Healthcare_SVM 스토리지 VM에 있는 6개의 ESXi 부팅 LUN을 보여주는 ONTAP System Manager의 스크린샷"]

. LUN을 iSCSI 이니시에이터 그룹에 매핑합니다.
+
image:flexpod-genomics-image11.png["ESXi_Boot_Lun_1이 GenomicsESXi_1 iSCSI 이니시에이터 그룹에 매핑된 것을 보여주는 스크린샷"]

+
image:flexpod-genomics-image12.png["GenomicsESXi_2 이니시에이터 그룹에 매핑된 ESXi 부트 LUN 세부 정보를 보여주는 ONTAP System Manager 스크린샷"]

. iSCSI 부트를 사용하여 vSphere 7.0을 설치합니다.
. vCenter에 ESXi 호스트를 등록합니다.
+
image:flexpod-genomics-image13.png["ESXi 호스트 및 가상 머신이 포함된 Genomics 폴더를 보여주는 vSphere Client 인벤토리의 스크린샷"]

. ONTAP 스토리지에서 NFS 데이터 저장소 'infra_datastore_nfs'를 프로비저닝합니다.
+
image:flexpod-genomics-image14.png["선택된 Healthcare_Workload_NFS_Vol NFS 볼륨이 온라인 상태임을 보여주는 ONTAP System Manager 스크린샷"]

. vCenter에 데이터 저장소를 추가합니다.
+
image:flexpod-genomics-image16.png["vSphere Client에서 infra_datastore_nfs NFS 3 데이터스토어 요약을 보여주는 스크린샷"]

. vCenter를 사용하여 ESXi 호스트에 NFS 데이터 저장소를 추가합니다.
+
image:flexpod-genomics-image15.png["정상 상태로 연결된 infra_datastore_nfs 호스트를 표시하는 vSphere Client의 스크린샷"]
...
+
image:flexpod-genomics-image17.png["/mnt/genomics에 마운트된 /dev/sdb의 사용률이 2%인 것을 보여주는 Linux 터미널 스크린샷"]
